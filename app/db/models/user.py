...
class User(Base, UUIDMixin, TimestampMixin):
    ...
    county = Column(
        ...
        nullable=True,
        index=True,
        doc="Kenyan County for regional benchmarking",
    )
    is_active = Column(
        Boolean,
        default=True,
        nullable=False,
        doc="Designates whether this user should be treated as active",
    )
    is_superuser = Column(
        Boolean,
        default=False,
        nullable=False,
        doc="Designates whether this user has admin privileges",
    )
    preferences = Column(
        JSON,
        default={},
        nullable=True,
        doc="User's personalized settings and preferences",
    )

    # Relationships
    flocks = relationship(
        "Flock", back_populates="farmer", cascade="all, delete-orphan"
    )
    role = Column(
        String(50),
        default="FARMER",
        ...
        doc="User's role (ADMIN, MANAGER, VIEWER, FARMER)",
    )

    def __repr__(self):
        return (
            f"<User(email='{self.email}', name='{self.full_name}', role='{self.role}')>"
        )
