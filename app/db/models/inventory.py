...
class InventoryItem(Base, UUIDMixin, TimestampMixin):
    ...
    category = Column(
        String(50), nullable=False, doc="Category: feed, medicine, equipment, other"
    )
    quantity = Column(
        DECIMAL(10, 2), nullable=False, default=0, doc="Current stock level"
    )
    unit = Column(
        String(20), nullable=False, doc="Unit of measurement (kg, liters, pcs)"
    )
