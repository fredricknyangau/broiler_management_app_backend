...
class Farm(Base, UUIDMixin, TimestampMixin):
    ...
    name = Column(
        String(255), nullable=False, index=True, doc="Name/Label of the farm location"
    )
    location = Column(
        String(255), nullable=True, doc="Geographic location or address details"
    )
    owner_id = Column(
        UUID(as_uuid=True),
        ...
        nullable=False,
        index=True,
        doc="The User ID who owns this farm",
    )
    is_active = Column(
        Boolean,
        default=True,
        nullable=False,
        doc="Status flag for active management switches",
    )
