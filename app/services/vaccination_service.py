...
class VaccinationService(BaseEventService[VaccinationEvent]):
    ...
    async def get_upcoming_vaccinations(
        self, flock_id: UUID, days_ahead: int = 7
    ...
    async def get_vaccination_history(
        self, flock_id: UUID, disease_target: Optional[str] = None
    ) -> List[VaccinationEvent]:
        """Get vaccination history, optionally filtered by disease"""
        stmt = select(VaccinationEvent).filter(VaccinationEvent.flock_id == flock_id)

        if disease_target:
            stmt = stmt.filter(VaccinationEvent.disease_target == disease_target)

        result = await self.db.execute(
            stmt.order_by(VaccinationEvent.event_date.desc())
        )
        return result.scalars().all()

    async def has_received_vaccine(self, flock_id: UUID, vaccine_name: str) -> bool:
    ...
    async def generate_schedule(
        self, flock_id: UUID, start_date: date
    ) -> List[VaccinationEvent]:
