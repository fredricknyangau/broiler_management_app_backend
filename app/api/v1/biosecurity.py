...
from app.schemas.biosecurity import (BiosecurityCheckCreate,
                                     BiosecurityCheckResponse,
                                     BiosecurityCheckUpdate)

router = APIRouter()


@router.post(
    "/", response_model=BiosecurityCheckResponse, status_code=status.HTTP_201_CREATED
)
...
async def update_biosecurity_check(
    ...
    """
    result = await db.execute(
        select(BiosecurityCheck).filter(
            BiosecurityCheck.id == check_id,
            BiosecurityCheck.farmer_id == current_user.id,
        )
    )
