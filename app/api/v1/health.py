...
@router.post(
    "/consultations",
    response_model=VetConsultationResponse,
...
async def read_consultations(
    ...
    result = await db.execute(
        stmt.order_by(VetConsultation.visit_date.desc()).offset(skip).limit(limit)
    )
...
async def update_consultation(
    ...
    """
    result = await db.execute(
        select(VetConsultation).filter(
            VetConsultation.id == consultation_id,
            VetConsultation.farmer_id == current_user.id,
        )
    )
...
async def delete_consultation(
    ...
    """
    Delete a vet consultation record.
    """
    result = await db.execute(
        select(VetConsultation).filter(
            VetConsultation.id == consultation_id,
