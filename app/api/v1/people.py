...
async def create_supplier(
    *,
    ...
    Create new supplier.
    """
    supplier = Supplier(**supplier_in.model_dump(), user_id=current_user.id)
    db.add(supplier)
    await db.commit()
...
@router.post(
    ...
    dependencies=[Depends(deps.check_professional_subscription)],
)
async def create_employee(
    *,
    ...
    employee = Employee(**employee_in.model_dump(), user_id=current_user.id)
    db.add(employee)
    await db.commit()
    ...
    *,
    ...
    result = await db.execute(
        select(Employee).filter(
            Employee.id == employee_id, Employee.user_id == current_user.id
        )
    )
