...
from app.api.deps import (check_enterprise_subscription,
...
@router.post(
    "/{farm_id}/members", response_model=dict, status_code=status.HTTP_201_CREATED
)
async def add_farm_member(
    ...
    if farm.owner_id != current_user.id and current_user.role != "ADMIN":
        raise HTTPException(
            ...
            detail="Only the Farm Owner can manage members.",
        )
    ...
    exists = await db.execute(
        select(FarmMember).where(
            FarmMember.farm_id == farm_id, FarmMember.user_id == user_id
        )
    )
